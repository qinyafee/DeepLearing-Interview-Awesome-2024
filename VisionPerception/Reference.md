...
1. 特征点聚集现象。传统ORB使用固定阈值检测FAST角点，导致特征点集中在高对比度区域，分布不均匀。
2. 对噪声敏感BRIEF描述子直接比较像素灰度值，易受噪声干扰。
3. 尺度与旋转不变性不足。FAST角点检测缺乏尺度不变性，需依赖人工构建图像金字塔；旋转校正依赖灰度质心法，大角度旋转时匹配精度下降。
4. 重复纹理与光照变化下的性能差。在重复纹理区域易产生伪角点，光照变化导致描述子区分度降低
