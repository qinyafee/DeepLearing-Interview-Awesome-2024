...
分辨率由fx,fy决定
- 图像裁剪，FOV变小，分辨率不变，fx,fy不变，cx，cy变化(除以2?)；
- 图像缩小成1/2，FOV不变，分辨率降低，fx,fy,cx,cy都变成原来的1/2，还有0.5pixel问题
...
https://www.cnblogs.com/jymg/p/11693587.html
...
Tp = [[A t], [a^t, v]]
射影变换，它左上角为可逆矩阵 A,右上为平移 t,左下缩放 a^T 。2D 的射影变换一共有 8 个自由度,3D 则共有 15 个自由度。
...
# TODO 25. BEVFormer中的Spatial Cross-Attention的步骤？
...
车宽测距依赖车辆尺寸和参考点，而接地点测距需地面平坦和相机参数准确。通过动态校准、多传感器融合和深度学习技术，可有效放宽先验假设的限制，提升复杂场景下的测距鲁棒性。
一、车宽测距的先验知识与限制放宽手段
 1. 核心先验知识
   • 已知车宽或部件尺寸：基于几何成像模型的测距需已知车辆的实际宽度（如车体宽度、车牌宽度等）。例如，文献[26]利用车牌的实际宽度与成像像素宽度计算距离。
   • 固定参考点：通过车身特征（如后视镜、引擎盖边缘）作为视觉参考点，结合其在图像中的像素位置推算车宽。
   • 标准化数据：依赖车辆规格手册中提供的车宽参数（可能不包含后视镜）。

 2. 先验不成立的应对手段
   • 动态校准：当车型或车宽未知时，可通过激光雷达或双目视觉实时测量车宽，并反馈至测距模型。
   • 多特征融合：结合车牌、车轮、车灯等多部件检测结果，通过加权平均降低单一特征失效的影响。
   • 在线学习：利用行驶中采集的车辆尺寸数据，动态更新回归模型参数（如文献[5]的逆向建模思路）。

二、接地点测距的先验知识与限制放宽手段
 1. 核心先验知识
   • 平坦地面假设：基于逆透视变换（IPM）的测距需假设地面为平面，否则会导致投影畸变。
   • 固定相机参数：需已知相机高度、俯仰角等参数以构建几何投影模型。
   • 接地点可见性：依赖车轮与地面的接触点（接地点）在图像中清晰可辨。

 2. 先验不成立的应对手段
   • 动态姿态补偿：通过惯性测量单元（IMU）实时获取车辆俯仰角/横滚角，修正IPM变换矩阵。
   • 深度估计辅助：结合单目深度估计网络（如Monodepth2）预测地形高度，缓解非平坦地面的影响。
   • 多帧时序融合：利用历史帧中的接地点运动轨迹，通过卡尔曼滤波或LSTM预测当前接地点位置。

三、两类测距方法的对比与适用场景
| 方法       | 先验依赖程度 | 典型场景               | 鲁棒性提升策略                     |
|----------------|--------------|------------------------|-----------------------------------|
| 车宽测距   | 高           | 结构化道路、标准车型   | 多传感器融合 + 动态参考点校准 |
| 接地点测距 | 中           | 非结构化地形、动态路面 | IMU姿态补偿 + 深度估计增强 |
...
# 31. 车辆行驶过程中 Pitch 角度的三种方法？
 ...
 1. 基于灭点（Vanishing Point）的视觉估计法
• 原理：利用车道线或道路平行线在图像中的消失点位置变化与 Pitch 角的几何关系
  • 适用场景：结构化道路（车道线清晰）、车辆直行状态。
  • 限制：依赖车道线检测精度，弯道或遮挡场景失效。

 1. 基于惯性传感器（IMU/加速度计）的运动学估计法
• 原理：利用加速度传感器测量重力分量。当车辆静止或匀速时，加速度计输出的垂直方向分量包含重力沿坡度的分力，通过几何分解可计算 Pitch 角。
  • 公式示例：纵向坡度角 \(\alpha = \arcsin\left(\frac{a_x}{g}\right)\)，其中 \(a_x\) 为加速度计测得的纵向加速度，\(g\) 为重力加速度。
  • 适用场景：动态行驶（含加减速）、非结构化道路。
  • 限制：急加速/减速时精度下降，需融合其他信号（如轮速差法）。

 1. 基于地面平面拟合与逆透视变换（IPM）的视觉估计法
• 原理：通过单目或双目相机拟合地面平面模型，结合相机外参（高度、安装角）计算 Pitch 角。若已知地面为平面，可通过逆透视变换将图像投影到鸟瞰图，反推相机姿态。
  • 实现步骤：
    1. 地面拟合：利用车道线、消失点或深度网络（如 Monodepth2）提取地面特征。
    2. 几何投影：根据相机高度 \(H\) 和主点坐标，构建 IPM 映射关系，推导 Pitch 角。
  • 公式示例：\(Pitch = \arctan\left(\frac{H}{Z}\right)\)，其中 \(Z\) 为相机到地面拟合平面的垂直距离。
  • 适用场景：平坦路面、已知相机外参标定值。
  • 限制：非平坦地形（如起伏路面）导致误差，需动态补偿（如 IMU 反馈）。

[单目相机的测距](https://funnywii.com/archives/%E5%8D%95%E7%9B%AE%E7%9B%B8%E6%9C%BA%E7%9A%84%E6%B5%8B%E8%B7%9D-thedistanceestimationofmonocularcamera)【链接失效】
